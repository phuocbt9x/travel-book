...
final class FileDownloadProvider
{
  ...
  {
    // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- URL-based file download; nonce would break shareable download links.
  ...
  public function authCheckandFrceDownloadHelper()
  {
    ...
    if (!empty($post->post_content)) {
      ...
      if (!empty($regexMatchGroups[2]) && in_array('bitforms-frontend-file', $regexMatchGroups[2]) && is_user_logged_in()) {
        ...
        if ($file) {
          // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- URL-based file access; auth handled via is_user_logged_in() check above.
