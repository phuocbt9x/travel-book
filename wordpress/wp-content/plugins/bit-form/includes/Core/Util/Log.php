  ...
  public static function debug_log($log)
  {
    ...
    if (WP_DEBUG === true) {
      if (is_array($log) || is_object($log)) {
        ...
        $log_msg = print_r($log, true); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
      } else {
        $log_msg = $log . ' in ' . $trace_info;
      }

      error_log($log_msg); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
    }
  }
