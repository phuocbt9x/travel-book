  ...
  public function updateFormEntry($updatedValue, $formID, $entryID)
  {
    ...
    if (count($file_fields) > 0) {
      ...
      foreach ($file_fields as $field_key) {
          ...
          if ($repeaterFldKey) {
            $repeaterExistData = $entryMeta->get(
              ...
              [
                ...
                'meta_key'               => $repeaterFldKey, // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
              ]
            );
            ...
            $file_exists = $entryMeta->get(
              ...
              [
                'bitforms_form_entry_id' => $entryID,
                'meta_key'               => $field_key, // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
              ]
            );
        ...
        // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- File upload data cannot be sanitized with sanitize_text_field
    ...
    foreach ($fields as $fieldKey => $fieldData) {
      if (array_key_exists('name', $fieldData)) {
        ...
        if (array_key_exists($fldName, $_POST)) {
          // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Form field data may contain HTML or complex values
          $temp = wp_unslash($_POST[$fldName]);
          unset($_POST[$fldName]);
          $_POST[$fieldKey] = $temp;
