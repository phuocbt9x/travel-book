...
final class IntegrationHandler
{
  ...
  public static function maybeSetCronForIntegration($workFlowReturnedData, $opType, $isFormRequest = true)
  {
    ...
    if (function_exists('fastcgi_finish_request') || !wp_doing_ajax()) {
      ...
      ob_start();
      // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $response contains wp_json_encode() output intended for JSON context and $workFlowReturnedData['message'] is escaped internally and safe to output.
      ...
      if (isset($workFlowReturnedData['integrationRun']) && !$workFlowReturnedData['integrationRun']) {
        ...
        if (!is_wp_error($updatedStatus)) {
          if ($workFlowReturnedData['dflt_template']) {
            // $triggerData['fields'] = $workFlowReturnedData['fields'];
            do_action('bf_double_optin_confirmation', $workFlowReturnedData['integrationDetails'], $triggerData); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
