...
class StripeField
{
  ...
  private static function field($field, $rowID, $field_name, $form_atomic_Cls_map, $formID, $error = null, $value = null)
  {
    ...
    if ($fieldHelpers->property_exists_nested($field, 'config->linkAuthentication->active', true)) {
      $linkAuthentication = sprintf('<div class="%1$s-stripe-auth-wrp"></div>', $rowID);
    } else {
      $linkAuthentication = '';
    }
    ...
    if ($fieldHelpers->property_exists_nested($field, 'config->address->active', true) && $fieldHelpers->property_exists_nested($field, 'config->address->mode', 'billing')) {
      $address = sprintf('<div class="%1$s-stripe-addr-wrp"></div>', $rowID);
    } else {
      $address = '';
    }

    $wrapperClass = $fieldHelpers->getConversationalCls('stripe-wrp');
    $buttonClass = $fieldHelpers->getConversationalCls('stripe-btn');
    $iconClass = $fieldHelpers->getConversationalCls('stripe-icn');
    $fieldClass = $fieldHelpers->getConversationalCls('stripe-fld');
    $buttonText = $fieldHelpers->kses_post($field->txt);

    return sprintf(
      '<div class="%1$s">
        <button type="button" class="%2$s">
          <svg xmlns="http://www.w3.org/2000/svg" class="%3$s" viewBox="0 0 24 24">
          ...
          <svg
            width="25"
            height="25"
            class="stripe-btn-spinner d-none"
            version="1.1"
            id="L9"
