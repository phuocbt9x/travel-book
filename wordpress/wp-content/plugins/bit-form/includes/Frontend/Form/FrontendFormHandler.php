...
final class FrontendFormHandler
{
  ...
  private function addInlineScript($code, $handle = '', $position = 'after')
  {
    ...
    if (!wp_script_is($scriptHandle)) {
      wp_register_script($scriptHandle, '', [], BITFORMS_VERSION, true);
  ...
  private function addInlineStyle($code, $handle = '')
  {
    ...
    if (!wp_style_is($styleHandle)) {
      wp_register_style($styleHandle, '', [], BITFORMS_VERSION);
  ...
  private function getValuesFromQueryParams()
  {
    ...
    if (isset($_SERVER['QUERY_STRING']) && !empty($_SERVER['QUERY_STRING'])) {
      // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- QUERY_STRING used with explode/urldecode; sanitize_text_field would break URL encoding
  ...
  public function handleFrontendRenderRequest($atts)
  {
    ...
    if ($captchaV3Settings && !empty($reCAPTCHA->siteKey)) {
      ...
      // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion, WordPress.WP.EnqueuedResourceParameters.NotInFooter -- External Google reCAPTCHA script; no version control available, must load in head.
    ...
    ?>
       <script id="bit-form-bf-globals-<?php echo esc_attr($FormIdentifier); ?>">
         <?php
    ...
    foreach ($formIds as $formID) {
      ...
      if (!wp_style_is('bitform-style-' . $newFormId) && is_readable(BITFORMS_CONTENT_DIR . '/form-styles/bitform-' . $newFormId . '.css')) {
        ...
        if ($isPageBuilder) {
          ...
          // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS controlled by the administrator, safe to output as-is.
      ...
      if (!wp_style_is('bitform-style-custom-' . $formID) && is_readable(BITFORMS_CONTENT_DIR . '/form-styles/bitform-custom-' . $formID . '.css')) {
        ...
        if ($isPageBuilder) {
          ...
          // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Custom CSS controlled by the administrator, safe to output as-is.
