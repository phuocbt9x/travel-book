	...
	<div class="wpte-repeater-block-holder wpte-glb-trp-infos-holdr">
		...
		if ( is_array( $default_trip_facts ) ) {
			foreach ( $default_trip_facts as $_id => $_trip_fact ) {
				...
				<div class="wpte-repeater-block wpte-sortable wpte-glb-trp-infos-row">
					<div class="wpte-field">
						<input type="hidden" name="wp_travel_engine_settings[default_trip_facts][<?php echo esc_attr( $_id ); ?>][fid]" value="<?php echo isset( $_trip_fact['fid'] ) ? esc_attr( $_trip_fact['fid'] ) : ''; ?>">
						<input type="text" name="wp_travel_engine_settings[default_trip_facts][<?php echo esc_attr( $_id ); ?>][field_id]" value="<?php echo isset( $_trip_fact['field_id'] ) ? esc_attr( $_trip_fact['field_id'] ) : ''; ?>" required>
					</div>
					<div class="wpte-icons-holder wpte-floated">
						<button class="wpte-add-icon"><?php echo ! empty( $fact_icon ) ? esc_html__( 'Update fact icon', 'wp-travel-engine' ) : esc_html__( 'Add fact icon', 'wp-travel-engine' ); ?></button>
						<span class="wpte-icon-preview">
						...
						<input class="trip-tabs-icon" type="hidden" name="wp_travel_engine_settings[default_trip_facts][<?php echo esc_attr( $_id ); ?>][field_icon]" value="<?php echo esc_attr( $fact_icon ); ?>">
					</div>
					<div class="wpte-trp-inf-fieldtyp wpte-field">
						<input type="text"
							readonly name="wp_travel_engine_settings[default_trip_facts][<?php echo esc_attr( $_id ); ?>][field_type]"
					...
					<div class="wpte-field">
						<div class="input-placeholder">
							<input readonly type="text" name="wp_travel_engine_settings[default_trip_facts][<?php echo esc_attr( $_id ); ?>][input_placeholder]" value="<?php echo isset( $_trip_fact['input_placeholder'] ) ? esc_attr( $_trip_fact['input_placeholder'] ) : ''; ?>">
						</div>
					</div>
					<div class="wpte-system-btns">
	...
	<script>
		;(function() {
			...
			window.addEventListener('load', function() {
				var index = 0;
				var _html = Object.values(highlights).length > 1 ? Object.values( highlights ).reduce(function(acc, curr){
					var _acc = ''
				...
				}) : Object.values(highlights).length == 1 ?  window.wp.template('wpte-highlight-item')({
					index: index,
					highlight: highlights[0].highlight,
					helpText: highlights[0].help
				}) : ""
